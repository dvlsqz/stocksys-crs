                        ...
                        @foreach($det_escuelas_l as $det3)
                            @if($det3->escuela_id == $det->escuela_id)
                                <p> 
                                    ...
                                    <b>Peso de Raciones en Gramos:</b> {{ number_format( ($det3->dias * $det3->total_personas * $det3->peso_racion), 2, '.', ',') }} <br> @php($total_peso_ruta = $total_peso_ruta + ($det3->dias * $det3->total_personas * $det3->peso_racion)  ) 
                                    <b>Peso de Raciones en Quintales:</b> {{ number_format( (($det3->dias * $det3->total_personas * $det3->peso_racion)/453.59237)/100, 2, '.', ',' ) }} <br>  @php($total_peso_ruta_quintales = $total_peso_ruta_quintales + ((($det3->dias * $det3->total_personas * $det3->peso_racion)/453.59237)/100))
                                    <b>Unidades de Racion:</b> {{ number_format( (($det3->dias * $det3->total_personas * $det3->peso_racion)/1000)/50, 2, '.', ',' ) }}
                                </p> 
                            @endif
                        @endforeach
                    </div>
                        ...
                        @foreach($det_escuelas_v_d as $det4)
                            @if($det4->escuela_id == $det->escuela_id)
                                <p> 
                                    ...
                                    <b>Peso de Raciones en Gramos:</b> {{ number_format( ($det4->dias * $det4->total_personas * $det4->peso_racion), 2, '.', ',' ) }} <br> @php($total_peso_ruta = $total_peso_ruta + ($det4->dias * $det4->total_personas * $det4->peso_racion)  ) 
                                    ...
                                    <b>Unidades de Racion:</b> {{ number_format( (($det4->dias * $det4->total_personas * $det4->peso_racion)/1000)/50, 2, '.', ',' ) }}
                                </p> 
                            @endif
                        @endforeach
